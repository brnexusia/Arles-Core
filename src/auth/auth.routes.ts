...
function authError(reply: FastifyReply, error: unknown) {
  ...
  if (code === 'TRIAL_ALREADY_USED') {
    return reply.code(409).send({ error: 'Este e-mail ou telefone já possui um cadastro anterior.', code });
  }
  if (code === 'EMAIL_INVALID') return reply.code(400).send({ error: 'Informe um e-mail válido.', code });
  if (code === 'PHONE_INVALID') return reply.code(400).send({ error: 'Informe um WhatsApp válido com DDD.', code });
