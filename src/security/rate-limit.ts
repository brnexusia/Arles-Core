...
function maybeCompanyId(value:string):string|null{
  return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value.trim())?value.trim():null;
}
...
export async function enforceRateLimit(
  ...
  const identityHash=rateIdentity(rule.identity);
  const key = `arles:rl:${rule.scope}:${identityHash}`;
  ...
  if (count > limit) {
    if(count===limit+1){
      void raiseSecurityAlert({
        companyId:maybeCompanyId(rule.identity),
        kind:'rate_limit_exceeded',
        severity:/auth:register|billing:beauty:activate|beauty:ai/.test(rule.scope)?'warning':'info',
        fingerprint:`rate:${rule.scope}:${identityHash}`,
        metadata:{scope:rule.scope,limit,window_seconds:window}
      }).catch(()=>undefined);
    }
...
export async function enforceIpLimit(
  ...
  await enforceRateLimit({scope,limit,windowSeconds,identity:requestIp(request)}, reply);
}
