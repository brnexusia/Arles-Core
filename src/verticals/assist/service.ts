...
export class AssistService {
  async overview(companyId: string) {
    const [metrics, recent] = await Promise.all([
      db.query(`select
        count(*) filter(where status in ('triage','quoted','awaiting_approval','confirmed'))::int as open_quotes,
  ...
  async latestForPhone(companyId: string, phoneInput: string) {
    const phone = clean(phoneInput).replace(/\D/g,'');
  ...
  async upsertConversationOrder(input: {companyId:string;phone:string;name:string;messageId:string;channel?:string;equipment?:string;brand?:string;model?:string;problem?:string;service?:any}) {
    ...
    const active = await db.query<{id:string;status:string}>(`select id::text,status from assist_orders where company_id=$1 and customer_phone=$2
      and status in ('triage','quoted','awaiting_approval','confirmed') order by updated_at desc limit 1`, [input.companyId,phone]);
    const service = input.service;
    const priced = service && service.pricing_mode !== 'diagnosis' && !service.requires_diagnosis;
    const patch: Record<string, unknown> = {};
    if (input.name) patch.customer_name = input.name;
    if (input.equipment) patch.equipment_type = input.equipment;
    ...
    if (active.rows[0]) {
      await this.updateOrder(input.companyId, active.rows[0].id, patch);
      return { id: active.rows[0].id, ...patch };
    }

    return this.createOrder(input.companyId, {
      ...patch,
      customer_phone: phone,
      channel: input.channel || 'whatsapp',
      source_message_id: input.messageId,
      status: priced ? 'quoted' : 'triage'
    });
  }
