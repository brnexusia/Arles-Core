...
function historicalBoundaryCue(value: string): boolean {
  return /\b(?:desde o inicio|desde que comecei|desde sempre|vida toda|todo o historico|historico inteiro|historico completo)\b/.test(value);
}

function explicitAllTimeUntilNowCue(value: string): boolean {
  return /\b(?:tudo que|tudo o que|de tudo|todo o historico|historico inteiro|historico completo)\b.*\b(?:ate hoje|ate agora)\b/.test(value)
    || /\b(?:ate hoje|ate agora)\b.*\b(?:de tudo|todo o historico|historico inteiro|historico completo)\b/.test(value);
}
...
export function hasCashExplicitAggregatePeriod(input: string): boolean {
  ...
  if (!value || historicalBoundaryCue(value) || explicitAllTimeUntilNowCue(value)) return false;
...
export function parseCashAggregateIntent(input: string): CashAggregateIntent | null {
  ...
  if (historicalBoundaryCue(value) || explicitAllTimeUntilNowCue(value)) {
