...
function hasSpecificFilter(value: string): boolean {
  ...
  for (const match of value.matchAll(/\b(?:na|no|em|com|de|da|do|pela|pelo)\s+([a-z0-9][a-z0-9_-]*)/g)) {
    const token = match[1]; if (!token || generic.has(token) || MONTH_SET.has(token) || /^20\d{2}$/.test(token)) continue; return true;
  }
  return false;
}
function periodCanonical(value: string): string | null {
  // “até hoje/até agora” limita o histórico no presente; não significa período “hoje”.
  if (historicalBoundaryCue(value)) return null;
  ...
  if (month?.[1]) { const name = month[1] === 'marco' ? 'março' : month[1]; return month[2] ? `${name} de ${month[2]}` : name; }
  ...
  const fullDate = value.match(/\b(?:dia\s*)?(\d{1,2}[\/-]\d{1,2}(?:[\/-]\d{2,4})?)\b/); if (fullDate?.[1]) return fullDate[1];
  return null;
}
export function hasCashExplicitAggregatePeriod(input: string): boolean {
  const value = normalize(input); if (!value || historicalBoundaryCue(value)) return false; return periodCanonical(value) !== null;
}
export function parseCashAggregateIntent(input: string): CashAggregateIntent | null {
  ...
  const genericSummary = /\b(?:balanco|resumo|fechamento)\b/.test(value) || (aggregate && /\btudo\b/.test(value));
  ...
  const historical = allTimeCue(value) || (records && aggregate)
