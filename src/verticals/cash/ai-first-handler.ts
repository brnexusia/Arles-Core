...
import { cashConversationMemorySize, loadCashConversationMemory } from './conversation-memory.js';
import { rememberCashQueryContext } from './conversation-state.js';
import { executeCashContextualCalculation } from './contextual-calculation.js';
...
const HelpSectionSchema = z.enum(['menu', 'register', 'query', 'pockets', 'forecasts', 'manage', 'reports', 'plans']);
const CalculationSchema = z.object({
  base_mode: z.enum(['zero', 'current_balance', 'explicit']),
  explicit_base: z.number().finite().nullable(),
  operations: z.array(z.object({ type: z.enum(['income', 'expense']), amount: z.number().positive() })).min(1).max(30)
});
const SemanticSchema = z.object({
  intent: z.enum([
    'transaction', 'mixed', 'query', 'balance', 'calculation', 'projection', 'pocket', 'forecast_schedule',
    'forecast_query', 'history', 'weekly_report', 'monthly_report', 'edit', 'delete', 'undo', 'help', 'plans',
    'trial', 'categories', 'schedule', 'confirmation', 'cancellation', 'acknowledgement', 'unknown'
  ]),
...
function text(value: string): VerticalResult { return { actions: [{ type: 'text', text: value }] }; }
function normalize(value: string): string { return String(value ?? '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim().replace(/\s+/g, ' '); }
...
export function cashSocialReply(kind: SemanticIntent['social_kind'] | null | undefined): string {
  if (kind === 'greeting') return 'Oi! 😊 Como posso te ajudar?'; if (kind === 'thanks') return 'Por nada! 😊';
  if (kind === 'farewell') return 'Até mais! 👋'; if (kind === 'wellbeing') return 'Tudo certo por aqui 😊 E com você?'; return 'Certo 👍';
}
function estimatedNanoCostUsd(response: unknown): number {
  const usage = (response as any)?.usage; return (Number(usage?.input_tokens ?? 0) * 0.05 + Number(usage?.output_tokens ?? 0) * 0.40) / 1_000_000;
}
...
async function semanticRoute(context: VerticalContext): Promise<SemanticRouteResult | null> {
  ...
  const hasCurrentMessage = Boolean(context.message.messageId
  ...
  try {
    const response = await client.responses.parse({
      model: env.cashOpenaiModel, reasoning: { effort: 'minimal' }, max_output_tokens: 520,
      input: [{ role: 'system', content: [
        ...
        'INTENÇÕES: transaction=registrar dinheiro real; query=consultar registros; balance=saldo atual; calculation=conta com valores citados/contextuais sem registrar; projection=simulação hipotética; pocket=cofrinhos; forecast_schedule=SALVAR/AGENDAR previsão; forecast_query=consultar previsões; history=histórico; weekly_report/monthly_report=relatórios; edit/delete/undo=gestão; help/plans/trial/categories/schedule=administrativo; confirmation/cancellation=pendência; acknowledgement=social; unknown=ambígua.',
        ...
        'Quando intent não for calculation, calculation deve ser null. Em rewritten_text, preserve dados necessários aos executores legados. clarification só deve existir em unknown.',
    ...
    const parsed = response.output_parsed; if (!parsed) return null;
    return { parsed, estimatedUsd: estimatedNanoCostUsd(response) };
  } catch (error) { console.error('[CashAI] falha na camada semântica contextual:', error); return null; }
}

function canonical(intent: SemanticIntent['intent']): string | null {
  const map: Partial<Record<SemanticIntent['intent'], string>> = { weekly_report: 'relatório semanal', monthly_report: 'relatório mensal', undo: 'coloca o último registro excluído de volta', plans: 'planos', trial: 'trial', categories: 'categorias', schedule: 'quando recebo os relatórios?' };
  return map[intent] ?? null;
}
async function executePocketCanonical(context: VerticalContext, rewritten: string): Promise<VerticalResult | null> {
  const c = { ...context, combinedText: rewritten };
  return await handleCashPocketCommand(c) ?? await handleCashPocketClosingFlow(c) ?? await handleCashPocketReceivable(c)
    ?? await handleCashPocketTransfer(c) ?? await handleCashPocketOrganization(c) ?? await handleCashPocketContextCommand(c);
}
...
export class CashAiFirstHandler implements VerticalHandler {
  async handle(context: VerticalContext): Promise<VerticalResult | null> {
    const semantic = await semanticRoute(context); if (!semantic) return text('Não consegui interpretar sua mensagem agora. Tente novamente em instantes, por favor.');
    ...
    if (understood.intent === 'confirmation' || understood.intent === 'cancellation') {
      ...
      if (pending) return pending; return text(understood.intent === 'confirmation' ? 'Certo 👍' : 'Tudo bem 👍');
    }
    ...
    if (understood.intent === 'transaction') return await executeCashAiTransaction(context, understood.rewritten_text, semantic.estimatedUsd) ?? text('Faltou alguma informação para registrar. Pode me dizer o valor e o que aconteceu?');
    if (understood.intent === 'calculation') return await executeCashContextualCalculation(context, understood.calculation);
    ...
    if (understood.intent === 'history') { await rememberCashQueryContext(context.company.id, context.message.phone, cashHistoryContextMarker); return await cashConversationHandler.handle({ ...context, combinedText: 'histórico' }); }
    if (understood.intent === 'balance') return await handleCashLedgerDeterministic({ ...context, combinedText: 'saldo' });
    if (understood.intent === 'projection') { const rewritten = understood.rewritten_text?.trim() || context.combinedText; return await handleCashLedgerDeterministic({ ...context, combinedText: rewritten }) ?? text('Não consegui calcular a simulação com os dados informados.'); }
    if (understood.intent === 'delete') {
      if (context.message.quotedMessageId || context.message.quotedText) { const q = await handleCashQuotedManagement({ ...context, combinedText: understood.rewritten_text?.trim() || context.combinedText }); if (q) return q; }
      return await executeCashAiDeletion(context, understood.rewritten_text, semantic.estimatedUsd);
    }
    if (understood.intent === 'pocket') { const rewritten = understood.rewritten_text?.trim() || context.combinedText; return await executePocketCanonical(context, rewritten) ?? text('Diga qual cofrinho e o que você quer fazer com ele.'); }
    if (understood.intent === 'forecast_schedule' || understood.intent === 'forecast_query') { const rewritten = understood.rewritten_text?.trim() || context.combinedText; return await handleCashScheduleDeterministic({ ...context, combinedText: rewritten }) ?? text('Informe o valor e a data ou recorrência da previsão.'); }
    if (understood.intent === 'mixed') { const rewritten = understood.rewritten_text?.trim() || context.combinedText; return await handleCashMixedNarrativeGate({ ...context, combinedText: rewritten }) ?? text('Pode separar esse pedido em duas frases para eu concluir certinho?'); }
    if (understood.intent === 'edit') {
      const rewritten = understood.rewritten_text?.trim() || context.combinedText;
      if (context.message.quotedMessageId || context.message.quotedText) { const q = await handleCashQuotedManagement({ ...context, combinedText: rewritten }); if (q) return q; }
    ...
    if (understood.intent === 'query') { const rewritten = understood.rewritten_text?.trim() || context.combinedText; return await cashBroadHandler.handle({ ...context, combinedText: rewritten }) ?? text('Não encontrei dados para essa consulta.'); }
    const command = canonical(understood.intent); if (command) return await cashBroadHandler.handle({ ...context, combinedText: command }) ?? text('Não consegui concluir essa ação agora.');
